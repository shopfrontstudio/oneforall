...
import { CATEGORY_META_MAP, PATHWAY_LABELS, PHASE1_SERVICE_MAP, serviceAvailability } from '@/lib/catalogue';
...
export default function ServiceDetail() {
  ...
  return (
    <div className="space-y-6">
      ...
      <header className="glass rounded-3xl p-5 sm:p-8"><p className="text-xs font-bold uppercase tracking-[0.14em] text-terracotta">{CATEGORY_META_MAP[service.category]?.name}</p><h1 className="mt-2 text-3xl font-semibold">{service.name}</h1><p className="mt-3 text-lg font-medium text-eucalyptus-deep">{PATHWAY_LABELS[service.pathway]}</p>{service.adults_only && <p className="mt-3 inline-flex rounded-full bg-sandstone/60 px-3 py-1 text-sm font-semibold">Adults only · low-risk services</p>}{service.category === 'pest-control' && <p className="mt-3 text-sm text-muted-foreground">Diagnostic first: treatment is never performed through the initial assessment route.</p>}<div className="mt-5 rounded-2xl border border-sandstone-deep/50 bg-sandstone/30 p-4" role="status"><b>Not accepting public requests.</b><span className="block text-sm text-muted-foreground">The release gate for this service is off.</span></div></header>
      ...
      <section className="glass rounded-2xl p-5"><h2 className="flex items-center gap-2 font-semibold"><AlertTriangle size={18} className="text-terracotta" />Before continuing</h2><p className="mt-2 text-sm text-muted-foreground">Unknown or mixed scope is held for manual review. Regulated or prohibited scope is blocked. Eligibility is service-specific and checked again before quoting and booking.</p><Link to={PUBLIC_PATHS.intake(service.key)} aria-disabled={unavailable && !IS_DEV_PREVIEW} className="mt-4 inline-flex rounded-xl bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-primary/25">{IS_DEV_PREVIEW ? 'Preview request intake' : 'Review intake questions'}</Link>{IS_DEV_PREVIEW && <p className="mt-2 text-xs font-semibold text-terracotta">Local QA preview only · creates no record</p>}</section>
    </div>
  );
}
