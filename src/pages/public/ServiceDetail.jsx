...
export default function ServiceDetail() {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="glass rounded-2xl p-5"><h2 className="flex items-center gap-2 font-semibold"><AlertTriangle size={18} className="text-terracotta" />Before continuing</h2><p className="mt-2 text-sm text-muted-foreground">Unknown or mixed scope is held for manual review. Regulated or prohibited scope is blocked. Eligibility is service-specific and checked again before quoting and booking.</p>{!unavailable || IS_DEV_PREVIEW ? <Link to={PUBLIC_PATHS.intake(service.key)} className="mt-4 inline-flex rounded-xl bg-action px-5 py-3 text-sm font-semibold text-action-foreground shadow-sm transition-colors hover:bg-action-deep hover:text-white focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-primary/25">{IS_DEV_PREVIEW ? 'Preview request intake' : 'Start service request'}</Link> : <p className="mt-4 rounded-xl border border-border bg-white/60 px-4 py-3 text-sm font-semibold" role="status">Requests are temporarily paused for this service.</p>}{IS_DEV_PREVIEW && <p className="mt-2 text-xs font-semibold text-terracotta">Local QA preview only · creates no record</p>}</section>
    </div>
  );
}
