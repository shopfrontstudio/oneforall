  ...
  return (
    <div className="mx-auto max-w-3xl space-y-5">
      ...
      <form ref={formRef} onSubmit={submit} noValidate className="glass space-y-6 rounded-3xl p-5 sm:p-7">
        ...
        <div className="flex flex-col gap-2 sm:flex-row"><button type="submit" disabled={submitting} className="inline-flex min-h-11 flex-1 items-center justify-center rounded-xl bg-action px-5 py-3 text-sm font-semibold text-action-foreground shadow-sm transition-colors hover:bg-action-deep hover:text-white focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-primary/25 disabled:opacity-60">{submitting ? <><Loader2 size={16} className="mr-2 animate-spin" />Sending request…</> : IS_DEV_PREVIEW ? 'Preview request — no record' : 'Send private request'}</button><button type="button" disabled={submitting} onClick={reset} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-border bg-white px-5 py-3 text-sm font-semibold focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-primary/25 disabled:opacity-60"><RotateCcw size={16} />Clear draft</button></div>
      </form>
    </div>
  );
}
