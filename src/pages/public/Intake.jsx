  ...
  return (
    <div className="mx-auto max-w-3xl space-y-5">
      ...
      <form ref={formRef} onSubmit={submit} noValidate className="glass space-y-6 rounded-3xl p-5 sm:p-7">
        ...
        {service.pathway === 'scheduled_or_recurring' && <fieldset className="space-y-4"><legend className="text-lg font-semibold">Timing preferences</legend><div><label htmlFor={`${formId}-date`} className="text-sm font-semibold">Preferred date</label><input id={`${formId}-date`} type="date" value={draft.preferred_date} onChange={(event) => set('preferred_date', event.target.value)} aria-invalid={Boolean(error('preferred_date'))} aria-describedby={error('preferred_date') ? `${formId}-date-error` : undefined} className="inp mt-2" /><FieldError id={`${formId}-date-error`}>{error('preferred_date')}</FieldError></div><div><label htmlFor={`${formId}-recurrence`} className="text-sm font-semibold">Frequency</label><select id={`${formId}-recurrence`} value={draft.recurrence} onChange={(event) => set('recurrence', event.target.value)} className="inp mt-2"><option value="once">One time</option><option value="weekly">Weekly</option><option value="fortnightly">Fortnightly</option><option value="monthly">Monthly</option></select></div></fieldset>}

        {service.pathway === 'managed_quote' && <fieldset className="space-y-3"><legend className="text-lg font-semibold">{service.key === 'general.guided_request' ? 'Optional triage context' : 'Managed quote context'}</legend><label htmlFor={`${formId}-photos`} className="flex cursor-pointer items-center gap-2 rounded-2xl border border-dashed border-border bg-white/60 p-4 text-sm font-semibold focus-within:ring-4 focus-within:ring-primary/20"><FileImage size={18} />Choose reference photos<input id={`${formId}-photos`} type="file" accept="image/*" multiple className="sr-only" onChange={(event) => set('photo_names', Array.from(event.target.files || []).slice(0, 8).map((file) => file.name))} /></label><p className="text-xs text-muted-foreground">Only filenames are retained across login in this foundation; files must be reattached later. Nothing uploads in preview.</p>{draft.photo_names.length > 0 && <p className="text-sm" role="status">Selected: {draft.photo_names.join(', ')}</p>}</fieldset>}
        ...
        <IntakeStatus state={visibleState} reason={assessment.scope?.reason} isAuthenticated={isAuthenticated} serviceKey={service.key} />
...
function IntakeStatus({ state, reason, isAuthenticated, serviceKey }) {
  ...
  if (state === 'manual_review') return <div className="rounded-2xl border border-sandstone-deep/40 bg-sandstone/25 p-4 text-sm" role="status"><p className="flex items-center gap-2 font-semibold"><Info size={17} />Manual review required</p><p className="mt-1 text-muted-foreground">This scope cannot proceed automatically. During a future live release, operations would review it before matching.</p></div>;
  ...
  if (state === 'unavailable') return <div className="rounded-2xl border border-sandstone-deep/40 bg-sandstone/25 p-4 text-sm" role="status"><b>Not accepting public requests.</b>{!isAuthenticated && <p className="mt-2">When this service opens, <Link to={`/login?returnTo=${encodeURIComponent(`/request/${serviceKey}`)}`} className="font-semibold text-eucalyptus-deep underline">log in</Link> will be required only when saving or submitting.</p>}</div>;
  return null;
}
