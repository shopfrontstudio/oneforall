...
import { PHASE1_SERVICES, PHASE1_SERVICE_MAP, RELEASE_FLAGS_OFF } from '../src/domain/catalogue.js';
...
test('all seven release controls are false for every service', () => {
  ...
  assert.deepEqual(Object.keys(RELEASE_FLAGS_OFF), keys);
  for (const service of PHASE1_SERVICES) {
    assert.deepEqual(service.flags, RELEASE_FLAGS_OFF);
    assert.equal(serviceAvailability(service), 'not_accepting_requests');
  }
});
